...
/**
 * com.tracktopell.gadget.viewmywebcam.ImageCompare
...
public class ImageCompare {
	...
	public static void main(String[] args) {
		...
		ImageCompare ic = new ImageCompare(args[0], args[1]);
		...
		if (!ic.match()) {
			saveJPG(ic.getChangeIndicator(), args[2]);
		}
	}
	...
	public void compare() {
		...
		for (int y = 0; y < comparey; y++) {
			...
			for (int x = 0; x < comparex; x++) {
				...
				if (diff > factorA) { // the difference in a certain region has passed the threshold value of factorA
					...
					ptsArrayList.add(new Point2D(x*blocksx+blocksx/2, y*blocksy+blocksy/2));
					this.match = false;
				}
		...
		if(ptsArrayList.size()>0){			
			...
			for(Point2D p2d: hull){
				...
				gc.fillOval((int)p2d.x()-blocksx/10,(int)p2d.y()-blocksy/10,
						blocksx/5,blocksy/5);

				if(p2dBefore!=null){
					gc.setColor(Color.YELLOW);
					gc.drawLine((int)p2dBefore.x()	,(int)p2dBefore.y()	,
										(int)p2d.x()	,(int)p2d.y()		);
				} else {
					p2dFirst = p2d;
				}
			...
			if(p2dFirst != p2dLast) {
				gc.setColor(Color.YELLOW);
				gc.drawLine((int)p2dFirst.x()	,(int)p2dFirst.y(),
									(int)p2dLast.x(),(int)p2dLast.y());
			}
		}
